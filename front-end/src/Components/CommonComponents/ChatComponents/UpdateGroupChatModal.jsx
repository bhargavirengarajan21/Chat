...
const UpdateGroupModal = ({ fetchMessages, fetchAgain, setFetchAgain, }) => {
  ...
  const { selectedChat, setSelectedChat, userInfo } = useConfig();
  ...
  const handleAddUser = async(user1) => {
    ...
    try {
      ...
      const { data } = await axios.put('/chat/addGroup', {chatid: selectedChat._id, userId: user1._id}, config);
      setSelectedChat(data);
      setLoading(false);
    } catch(error) {
      toast({
  ...
  const handleRemove = async (user1) => {
    console.log("enter",user1,selectedChat);
    ...
    try {
      ...
      user1._id === user1._id ? setSelectedChat() : setSelectedChat(data);
      setFetchAgain(!fetchAgain);
      fetchMessages();
      setLoading(false);
    } catch(error) {
          ...
          </FormControl>
          {
            loading ? (<Spinner size="lg" />) : (searchResult?.map((user) => (
              <UserListItem 
                key={userInfo._id}
                user={userInfo}
                handleFunction={()=> handleAddUser(userInfo)}
              />
            )))
          }
        </ModalBody>
